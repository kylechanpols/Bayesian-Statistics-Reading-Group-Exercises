...
library(pcse)
data('agl')
agl.lm <- lm(growth ~ lagg1 + opengdp + openex + openimp + central + leftc + leftc:central 
                      + as.factor(year), data=agl)
...
Load the `wbstats` package. This is the R API for the World Bank Indicators. Suppose you want to model GDP levels on oil rents and controlling for population, here the data essentially follows the TSCS structure and you would want to use multi-level models.\ 

## Time Series Analysis: A very short Primer
Before we dive into the mechanics of fitting the multi-level model, it is necessary to explain what we are giving to `brm()`. `brm()` will ask you two things when you want to specify an autogressive correlational structure: AR and MA. Below is a very brief description of what these two things are:\ 
...
library(forecast)
countries <- wbdat$iso3c %>% unique()
ex1 <- wbdat  %>% filter(iso3c == "USA")
ex1$date <- as.numeric(ex1$date)
ex1 <- ex1 %>% arrange(date)
gdp.ex1 <- ts(ex1, start=ex1$date[1], end=ex1$date[nrow(ex1)], frequency=1)
gdp.ex1 <- na.omit(gdp.ex1)
acf(gdp.ex1[,"gdp"]) #ACf for AR
pacf(gdp.ex1[,"gdp"]) #PACF for MA'
#arima.obj <- arima(growth.ex1[,"growth"], order=c(1,0,0))
```
...
```{r eval=F}
for(i in 1:10){
  d <- wbdat %>% filter(iso3c == countries[i])
  d$date <- as.numeric(d$date)
  d <- d %>% arrange(date)
  ts.dat <- ts(d, start=1970, end=2019, frequency=1) %>% na.omit()
  acf(ts.dat[,"gdp"])
}
...
```{r}
library(forecast)
arima101 <- arima(gdp.ex1[,"gdp"], order = c(1,0,1), method="ML")
...
for(i in 1:10){
  ...
  d <- d %>% arrange(date)
  ts.dat <- ts(d, start=1970, end=2019, frequency=1) %>% na.omit()
  auto.arima(ts.dat[,"gdp"]) %>% checkresiduals()
}
```
...
wb.brm <- brm(gdp ~ oilrents + log(pop),
          autocor= ~ brms::arma(time=date, gr=iso3c, p=2, q=2, cov=T),
...
`brm` follows the Bayesian approach in fitting the models. If you call `summary()`, it provides a summary of population-level effects drawn which you can interpret as a regular regression table. The extra element in the table is the 95% Critical Interval values, which is similar to the Frequentist Confidence Intervals. It also gives the random effects at Country and Time under the "Family Specific Parameter" tab. \ 
...
Now note that for time purposes, I have only let the model sample 1000 times from the posterior. This is far from sufficient! Below you should play around with different ARMA settings, as well as improving the number of draws from the posterior and the number of burn-in steps (e.g. 4000 is a good start). This would help reach model convergence since the MCMC algorithm needs to run for a while to achieve stationarity in the Markov Chain (and thus model convergence).
