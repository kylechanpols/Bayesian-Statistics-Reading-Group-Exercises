---
title: "Lab 12- IRT (2): Polytomous IRT"
author: "Kyle Chan"
...
```{r}
mod.grm1 <- mirt(data = science,
                 model = model1,
                 itemtype="graded",
                 SE=TRUE,
                 verbose=FALSE)
coefs.grm1 <- coef(mod.grm1, IRTpars=TRUE, simplify=TRUE)
items.grm1 <- as.data.frame(coefs.grm1$items)
```
...
```{r}
mod.nom1 <- mirt(data = science,
                 model = model1,
                 itemtype="nominal",
                 SE=TRUE,
                 verbose=FALSE)
coefs.nom1 <- coef(mod.nom1, IRTpars=TRUE, simplify=TRUE)
items.nom1 <- as.data.frame(coefs.nom1$items)
```
...
```{r}
two_traits <- "econ = 3,6 life = 1,4,7"
mod.pcm2 <- mirt(data = science,
                 model = two_traits,
                 itemtype="gpcm",
                 SE=TRUE,
                 verbose=FALSE)
itemplot(mod.pcm2, item=3)
