...
b <- coef(mod) #X Beta
varcov <- vcov(mod) #Variance-Covariance Matrix
...
plotdat_noerror <- as.data.frame(cbind(x, beta_ols[,"x"]))
colnames(plotdat_noerror) <- c("data", "beta")
plotdat_error <- as.data.frame(cbind(xh, beta_hat_ols[,"xh"]))
colnames(plotdat_error) <- c("data", "beta")
plotdat <- rbind(plotdat_noerror, plotdat_error)
plotdat$group <- c(rep("No Error",100), rep("Error", 100))

ggplot(plotdat, aes(x=beta, fill=group))+
  geom_density()+
  theme_bw()

#When X is measured with error, its associated beta coefficient is downward biased towards 0. 
#You can use the same MC technique to acquire a sample of beta hat given X hat = X - epislon and show the same conclusion.
...
# the spike at the top is the prior, which is very poor when fitted to the data. The Gibbs Sampler quickly got to the true parameters by the 20th iteration.
...
#there's more randomness when approxiamating the mean and the variance. This is due to
# the fact that we accept/ reject proposals with a probability using the ratio r.